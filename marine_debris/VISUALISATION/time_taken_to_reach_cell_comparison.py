...
def binModel(lon_bnd, lat_bnd, param, fh):
    ...
    min_days[min_days < 32768] = min_days[min_days < 32768]/30
...
def plotModel(data1, data2, fh):
    ...
    for i in range(2):
        ...
        cmap_bounds = np.linspace(0, 24, 25)
        cmap_norm = colors.BoundaryNorm(cmap_bounds, cmap.N)

        # Plot the colormesh
        hist = a0.pcolormesh(lon_bnd, lat_bnd, np.ma.masked_where(data[i] <= 24, data[i]), cmap=cmap, norm=cmap_norm,
                             vmin=0, vmax=24)
        ...
        cb.set_label('Minimum months at sea to Seychelles', size=12)
