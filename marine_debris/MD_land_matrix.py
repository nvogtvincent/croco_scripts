...
param = { # Runtime parameters
         ...
         # Analysis parameters
         'ls_d': 1825,    # Sinking timescale (days)
         ...
         'c_frac': 0.0,     # Fraction of coastal plastics

         'title': 'Debris sources for zero windage, l(s)=10a, l(b)=30d',
         'cmap': cmr.guppy_r,
...
dirs = {'script': os.path.dirname(os.path.realpath(__file__)),
        ...
        'traj': os.path.dirname(os.path.realpath(__file__)) + '/TRAJ/LAND/'}
...
fh = {'source_list': dirs['plastic'] + 'country_list.in',
      ...
      'traj':    sorted(glob(dirs['traj'] + '*FwdLand0000.nc')),
    ...
    for fhi, fh in tqdm(enumerate(fh_list), total=len(fh_list)):
        with Dataset(fh, mode='r') as nc:
            ...
            if n_traj:
                ...
                loss = (lb/(lb+ls))*(prior_mass - post_mass)
...
drift_time_bnds = np.arange(0, 1830, 5) # Days
...
for year in np.arange(param['y0'], param['y1']+1):
    for month in np.arange(12):
        ...
        yearmonth_fh = sorted(glob(dirs['traj'] + str(year) + '_' + str(month+1) + '_*.nc'))
        save_fh = dirs['traj'] + 'land_data_s' + str(param['ls_d']) + '_b' + str(param['lb_d']) + '_' + str(year) + '_' + str(month+1) + '.pkl'

        data, stats = convert_events(yearmonth_fh, param['dt'], param['ls'], param['lb'], 35,
                                     particles_per_file=325233, save_data=True,
                                     save_fh=save_fh)
